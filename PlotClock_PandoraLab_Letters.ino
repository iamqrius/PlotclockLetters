...
/* This is the start of the implementation of writing text, when the letter code below is implemented in the number() function this code shall be adapted and activated
      switch (minute()) {
        //write 1/4nach
        case 15:
          number(cfg.start_x, cfg.start_y, 1, 0.9);                       //  1
          number(cfg.start_x + 13, cfg.start_y, hour() % 10, 0.9);        //  /: 26047
          number(cfg.start_x + 22, cfg.start_y, 4, 0.9);                  //  4
          number(cfg.start_x + 28, cfg.start_y, minute() / 10, 0.9);      //  n: 26110
          number(cfg.start_x + 41, cfg.start_y, minute() % 10, 0.9);      //  a: 26097
          number(cfg.start_x + 41, cfg.start_y, minute() % 10, 0.9);      //  c: 26099
          number(cfg.start_x + 41, cfg.start_y, minute() % 10, 0.9);      //  h: 26104

        //write halb x
        case 30:
          number(cfg.start_x, cfg.start_y, hour() / 10, 0.9);             //  h: 26104
          number(cfg.start_x + 13, cfg.start_y, hour() % 10, 0.9);        //  a: 26097
          number(cfg.start_x + 22, cfg.start_y, 11, 0.9);                 //  l: 26108
          number(cfg.start_x + 28, cfg.start_y, minute() / 10, 0.9);      //  b: 26098
          number(cfg.start_x + 41, cfg.start_y, minute() % 10, 0.9);      //  [Stunde]
        ...
        case 45:
          ...
          number(cfg.start_x + 13, cfg.start_y, hour() % 10, 0.9);        //  /: 26047
          number(cfg.start_x + 22, cfg.start_y, 4, 0.9);                  //  4
          number(cfg.start_x + 28, cfg.start_y, minute() / 10, 0.9);      //  v: 26118
          number(cfg.start_x + 41, cfg.start_y, minute() % 10, 0.9);      //  o: 26111
          number(cfg.start_x + 41, cfg.start_y, minute() % 10, 0.9);      //  r: 26114
      }
*/
      ...
      //the double point in the middle btw. hour and minute
...
void number(float bx, float by, int num, float scale) {
  ...
  switch (num) {
    ...
    // this seems to be the double point in the middle
    case 11:
    ...
    // draw the / sign
    case 26047:
    //the code for the drawing here, based on structure of numbers, see above
    break;  
...
inline void interactive(char incoming) {
  ...
  switch (i_state) {
    ...
    case 5:
      ...
      if (incoming == '7' || incoming == 'q' || incoming == 'Q') {
